...
{
    SetStatus((E_SMARTCAR_STATUS)mode);
}
...
#else
void BalanceCar::SetIrPin(uint8_t pin )
#endif
{
    IrPin = pin;
...
#else
void BalanceCar::SetBuzzerPin(uint8_t pin )
#endif
{
  BuzzerPin = pin;
...
#else
void BalanceCar::SetRgbPin(uint8_t pin )
#endif
{
    RgbPin = pin;
    mRgb = new RGBLed(BC_RGB_PIN, RgbPin);
    mRgb->setNumber(2);
    // mRgb->reset();
}
...
#else
void BalanceCar::LightOn(E_RGB_INDEX index , long Color )
#endif
{
    // mRgb->reset();
    if (index == E_RGB_ALL) {
...
#else
void BalanceCar::LightOff(E_RGB_INDEX index)
#endif
{
    //  mRgb->reset();
    if (index == E_RGB_ALL) {
...
#else
int BalanceCar::SetPs2xPin(uint8_t clk, uint8_t cmd , uint8_t att, uint8_t dat)
#endif
{
...
#else
void BalanceCar::SetUltrasonicPin(uint8_t Trig_Pin , uint8_t Echo_Pin)
#endif
{
    EchoPin = Echo_Pin;
    TrigPin = Trig_Pin;
