...
{
    if (recv->function == E_CONTROL_MODE) {
...
byte ProtocolParser::GetRobotDirection()
{
    if (recv->function == E_ROBOT_CONTROL_DIRECTION ) {
...
{
    byte SendBuffer[24];
    if (send_dat == NULL || len > 90) {
        DEBUG_ERR("SendPackage error");
        return false;
    }
    unsigned short checksum = 0;
    byte *p_data = &SendBuffer[5];
    protocol_data_len = len;
    SendBuffer[0] = send_dat->start_code;
    SendBuffer[1] = send_dat->len;
    SendBuffer[2] = send_dat->type;
    SendBuffer[3] = send_dat->addr;
    SendBuffer[4] = send_dat->function;
    checksum = SendBuffer[1] + SendBuffer[2] + SendBuffer[3] + SendBuffer[4];
